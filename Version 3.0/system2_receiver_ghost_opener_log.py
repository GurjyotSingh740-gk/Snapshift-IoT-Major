# SnapShift — SYSTEM 2 (Receiver)
# Receives file from System 1 over TCP
# After file lands:  BTN2 = Finalize (keep + open)  |  BTN3 = Cancel (delete)
#
# NEW: While state == "approaching", ESP32 MOTION (MOTION:gz:gy) is used
#      to drag a "ghost" file card across System 2 screen, so the motion
#      visually continues from S1 to S2.
#
# Run: python system2_receiver.py
...
ESP_UDP_PORT  = 5007    # MOTION from ESP32 (used for ghost drag on S2)
SCREEN_W      = 1920
SCREEN_H      = 1080

# Motion sensitivity on System 2 (can be tweaked independently from S1)
S2_MOVE_SENS  = 3.0     # pixels per deg/s
S2_DEADBAND   = 1.5     # ignore tiny movements
# ─────────────────────────────────────────────────────────────────
...
file_received    = False
received_path    = ""
# NEW: prevent auto-open running multiple times for same file
auto_open_done   = False

# States:
# "idle"        : nothing happening
# "approaching" : S1 says window is at edge / coming
# "receiving"   : TCP bytes are being written
# "arrived"     : file saved fully, waiting BTN2/BTN3
# "finalizing"  : BTN2 pressed, opening Explorer
state = "idle"

# Animation position for card
anim_x   = float(SCREEN_W + 160)   # default off-screen at right
anim_y   = float(SCREEN_H / 2)
anim_vel = 0.0
SLIDE_TARGET = SCREEN_W * 0.38

# For ghost drag phase (approaching)
ghost_active = False
ghost_x      = float(80)           # start near left edge
ghost_y      = float(SCREEN_H / 2)

# Progress bar (0.0 → 1.0)
recv_progress = 0.0

# Latest ESP32 gyro motion (from MOTION:gz:gy)
gyro_z = 0.0
gyro_y = 0.0

LOG_FILE_S2 = "snapshift_s2_log.csv"

def log_s2(state_str, note=""):
    """
    Append a state change / progress event for System 2.
    ts (epoch), state, note
    """
    try:
        ts = time.time()
        with open(LOG_FILE_S2, "a", newline="") as f:
            w = csv.writer(f)
            w.writerow([ts, state_str, note])
...
def reset_all():
    ...
    global anim_x, anim_vel, ghost_active, ghost_x, ghost_y
    global gyro_z, gyro_y, auto_open_done
    ...
    gyro_z          = 0.0
    gyro_y          = 0.0
    auto_open_done  = False                # ← reset the flag

    print("[S2] RESET")

def open_received_file_once():
    """Open the received file in Explorer only once after save."""
    global auto_open_done, received_path
    if auto_open_done:
        return
    if received_path and os.path.exists(received_path):
        try:
            print(f"[S2] AUTO-OPEN: {received_path}")
            # Use Windows file association to open the file itself
            subprocess.Popen(f'start "" "{received_path}"', shell=True)
            auto_open_done = True
        except Exception as e:
            print(f"[S2] Auto-open error: {e}")
...
def tcp_file_receiver():
    ...
    global recv_progress, anim_x, anim_y, anim_vel, ghost_active, auto_open_done
    ...
    while True:
        try:
            ...
            # Once real bytes start, stop ghost mode and
            # hand over to normal slide-in animation path later.
            ghost_active = False
            ...
            save_path = os.path.join(SAVE_DIR, filename)
             ...
             # Now we know which file, so set state + log
            state = "receiving"
            recv_progress = 0.0
            log_s2("RECEIVING_START", filename)
            ...
            with open(save_path, "wb") as f:
                while received_bytes < filesize:
                    ...
                    # log every 10% jump for clean graph
                    if pct // 10 != last_logged_pct // 10:
                        log_s2("RECEIVING", f"{pct}%")
                        last_logged_pct = pct

                    print(
                        f"\r[S2]   {int(recv_progress*100):3d}%  "
                        f"{received_bytes}/{filesize}",
                        end="",
                        flush=True
                    )
            ...
            if received_bytes == filesize:
                ...
                file_received  = True
                state          = "arrived"
                log_s2("ARRIVED", save_path)

                # Start S2 slide-in animation from right,
                # but we also snap anim_x to wherever the ghost ended
                # so the motion feels continuous.
                if not (0 < ghost_x < SCREEN_W):
                    # if ghost never used, start from default
                    anim_x = float(SCREEN_W + 160)
                else:
                    anim_x = ghost_x
                ...
                anim_y   = ghost_y
                anim_vel = -28.0
                auto_open_done = False      # ensure fresh for this file

                 # NEW: auto-open immediately after a successful save
                open_received_file_once()
                

            else:
                print(f"\n[S2] ❌ Incomplete: {received_bytes}/{filesize}")
                log_s2("RECEIVE_INCOMPLETE", f"{received_bytes}/{filesize}")
                state = "idle"

        except Exception as e:
            print(f"[S2] TCP error: {e}")
            log_s2("TCP_ERROR", str(e))


def udp_notify_receiver():
    """Receive control messages from System 1."""
    global state, incoming_title, incoming_fname, anim_y, ghost_active, ghost_x, ghost_y
    ...
    while True:
        try:
            ...
            log_s2("UDP_ERROR", str(e))


def esp_udp_listener():
    """
    Listen for BTN2/BTN3 + MOTION from ESP32 on System 2 port.
    - While state == 'approaching' and ghost_active == True:
      use MOTION:gz:gy to move ghost_x/ghost_y (continuous drag on S2).
    - While state in ('arrived', 'finalizing'):
      BTN2/BTN3 work exactly as before.
    """
    global state, received_path, gyro_z, gyro_y, ghost_active, ghost_x, ghost_y

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind(("0.0.0.0", ESP_UDP_PORT))
    sock.settimeout(0.5)
    print(f"[S2] ESP32 port on UDP {ESP_UDP_PORT} (BTN2/BTN3 + MOTION)")
    ...
    while True:
        try:
            ...
            # MOTION:gz:gy  (same format as System 1)
            if msg.startswith("MOTION:"):
                ...
                if len(parts) == 3:
                    ...
                    # Only use motion for ghost drag while approaching
                    if state == "approaching" and ghost_active:
                        dx = gyro_z * S2_MOVE_SENS
                        dy = gyro_y * S2_MOVE_SENS
                        if abs(dx) < S2_DEADBAND:
                            dx = 0.0
                        if abs(dy) < S2_DEADBAND:
                            dy = 0.0
                        ...
                        # clamp to screen
                        ghost_x = max(80.0, min(SCREEN_W - 80.0, ghost_x))
                        ghost_y = max(80.0, min(SCREEN_H - 80.0, ghost_y))

                continue  # handled MOTION

            # BTN2 / BTN3 handling for arrived/finalizing (unchanged)
            if state == "arrived" or state == "finalizing":

                if msg == "RELEASE":     # BTN2 → Finalize
                    state = "finalizing"
                    print(f"[S2] ✅ FINALIZED: {received_path}")
                    log_s2("FINALIZE", received_path)
                    if received_path and os.path.exists(received_path):
                        # Open the file again (or bring app to front if already open)
                        subprocess.Popen(f'start "" "{received_path}"', shell=True)

                elif msg == "RESET":     # BTN3 → Cancel / delete
                    log_s2("DELETE_OR_RESET", received_path)
                    if received_path and os.path.exists(received_path):
            ...
            log_s2("ESP_UDP_ERROR", str(e))


def animation_ticker():
    """Slide-in physics for the file icon when state == 'arrived'."""
...
def draw_file_card(screen, x, y, label, font_sm, arrived, progress=1.0, ghost=False):
    ...
    if ghost:
        gc = (120, 160, 255, 60)   # slightly different tint for ghost
    else:
    ...
    if not arrived and progress < 1.0 and not ghost:
...
def run_overlay():
    ...
    while True:
        ...
        if state == "approaching":
            ...
            txt = font_big.render(
                f"📡  Incoming: {incoming_title[:46]}  —  Ghost drag active...",
                True, (255, 200, 60)
            )
        ...
        # ── File card / ghost drag ──────────────────────────
        if state == "approaching" and ghost_active:
            # Ghost drag card, driven by ESP32 MOTION
            draw_file_card(
                screen,
                ghost_x,
                ghost_y,
                incoming_fname or incoming_title or "File",
                font_sm,
                arrived=False,
                progress=1.0,
                ghost=True,
        ...
        if state in ("arrived", "finalizing"):
            ...
            draw_file_card(
                ...
                anim_x,
                anim_y,
                incoming_fname or incoming_title,
                font_sm,
                arrived=False,
...
if __name__ == "__main__":
    print("=" * 52)
    print("  SnapShift System 2 — Receiver (Ghost Drag)")
    print(f"  Save dir : {SAVE_DIR}")
    print(f"  TCP port : {HANDOFF_PORT}  (file bytes)")
    print(f"  UDP port : {NOTIFY_PORT}   (notify from S1)")
    print(f"  ESP port : {ESP_UDP_PORT}  (BTN2/BTN3 + MOTION from ESP32)")
