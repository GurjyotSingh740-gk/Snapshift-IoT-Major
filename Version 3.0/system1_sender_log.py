...
LOG_FILE = "snapshift_s1_log.csv"

UDP_PORT    = 5005
MY_IP       = "0.0.0.0"

SYSTEM2_IP   = "192.168.0.101"    # â† Change to System 2 IP -- Find system2 ip address on terminal by command ipconfig
...
def tcp_send_file(filepath):
    ...
    try:
        ...
        log_s1("SEND_DONE", filename)

    except Exception as e:
        print(f"\n[S1] âŒ Transfer failed: {e}")
        transfer_active = False
        log_s1("SEND_FAIL", str(e))
...
def do_transfer():
    ...
    if grabbed_file and not transfer_done:
        ...
        # LOG: send about to start
        log_s1("SEND_START", grabbed_file)
            ...
            # â”€â”€ Button 2 â†’ RELEASE or SEND â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if is_grabbed:
                    if at_edge and not transfer_done:
                        ...
                        log_s1("EDGE_SEND", grabbed_title)
                        do_transfer()
                        is_grabbed = False
                    else:
                        ...
                        log_s1("DROP", f"{grabbed_title} @ ({drag_x:.0f},{drag_y:.0f})")
                else:
    ...
    while True:
        if is_grabbed and grabbed_hwnd:
            ...
            if drag_x >= SCREEN_W - 280 and not transfer_done:
                ...
                log_s1("EDGE_APPROACH", grabbed_title)
...
def log_s1(state, note=""):
    """
    state: short code, e.g. IDLE, GRAB, DRAG, EDGE, SEND
    note:  optional extra info (window title, file name, etc.)
    """
    ts = time.time()
    with open(LOG_FILE, "a", newline="") as f:
        w = csv.writer(f)
        w.writerow([ts, state, note])
